...
public class IdpRestAPITest {
    ...
    public void testSetAttribute()
            throws IOException, ServletException, MessageEncodingException, XMLParserException, UnmarshallingException {

        final String user = "user";
        final String name = "foo";
        final String value = "bar";

        final User userObj = new User(user, DEFAULT_PASSWORD);
        userObj.setAuthorities(Arrays.asList("ROLE_USER"));
        restApiController.addUser(userObj);

        final Response respBefore = testHelper.doSamlLogin(user, DEFAULT_PASSWORD);
        assertFalse(testHelper.responseHasAttribute(name, value, respBefore));

        final Attribute attr = new Attribute();
        attr.setValue(value);

        restApiController.setAttribute(user, name, attr);

        final Response respAfter = testHelper.doSamlLogin(user, DEFAULT_PASSWORD);
    ...
    public void testRemoveAttribute() throws IOException, ServletException, MessageEncodingException, XMLParserException,
        ...
        restApiController.removeAttribute(user, name);
    ...
    public void testSetAuthentication() throws IOException, XMLParserException, ServletException, MessageEncodingException,
            UnmarshallingException {
        final Response response = testHelper.doSamlLogin("john.doe", "asdlkfjbdiufv");
        assertTrue(testHelper.responseHasAttribute("urn:mace:dir:attribute-def:uid", "john.doe", response));
    }
}
